...
| 主动识别 | 在标定中距离把物块依次移到画面左、中、右，各采 3 帧 |
| 类别复核 | 对三个真实视角的 YOLO 框内标签做 HOG/SVM 九帧投票，避免单视角夹错 |
| 最终对位 | 只有类别匹配时，才按框中心、宽度和高度误差进入可夹取范围 |
| 夹取条件 | 检测框稳定落入可夹取范围，且多视角类别与任务类别一致 |
...
| --- | --- | --- |
| 左 | `(291.60,317.50,355.40,390.00)` | `(0.4965,0.7399,0.1134,0.1756)` |
| 中 | `(291.60,317.55,355.40,388.60)` | `(0.4915,0.7332,0.1183,0.1730)` |
| 右 | `(291.60,317.55,355.40,388.60)` | `(0.5033,0.7442,0.1079,0.1660)` |
...
视觉模型类别顺序为 `food`、`daily`、`electronics`。模型及来源校验信息位于：
