...
2. 在另一个终端输入物品任务；
3. 程序判定物品类别，按左、中、右固定观察位依次读取摄像头；
4. YOLOv5 定位物块并完成视觉类别复核，底盘用图像误差闭环对位后夹取；
...
持续发布给 RViz 和 YOLO，未使用的深度相机改为按需启动。底盘插件设有
`0.10 s` 指令看门狗；局部规划器拒绝某一控制周期后，Gazebo 不会继续执行上一条
非零速度。
...
物块模型名会被随机生成器随机分配到三个区域，因此运行节点不再读取
`cube_0`、`cube_1`、`cube_2` 的 Gazebo 坐标，也不再按模型名推算取货区域。
...
| 观察位初判 | 物块一进入画面就开始分类，不再为了远距离分类先转动车身强制对中 |
| 快速排除 | 观察位连续 5 帧的 YOLO 类别一致且置信度不低于 `0.90` 时，可直接跳过非目标区域 |
| 安全回退 | 观察位置信度不足、类别波动或跟踪丢失时，不做提前排除，继续执行近距离严格复核 |
| 视觉对位 | 按框中心、宽度和高度误差，把当前区域的物块移入已标定可夹取范围 |
| 类别复核 | 远距离模板不参与快速排除；到可夹取画面尺度后连续采 7 帧，透视校正框内文字面并与三类标签模板投票 |
| 区域切换 | 类别不匹配时不夹取，继续到下一个固定观察位 |
| 夹取条件 | 检测框稳定落入可夹取范围，且近距离类别投票与任务类别一致 |
...
标定图像为 `640x480`。直接坐标逻辑移除前记录的轮廓框，以及同一图像上的
YOLO 可夹取目标如下：

| 区域 | 原始轮廓框 `(x1,y1,x2,y2)` | YOLO 目标 `(cx,cy,w,h)` |
| --- | --- | --- |
| 左 | `(291.60,317.50,355.40,390.00)` | `(0.5043,0.7373,0.0842,0.1462)` |
| 中 | `(291.60,317.55,355.40,388.60)` | `(0.5039,0.7372,0.0852,0.1461)` |
| 右 | `(291.60,317.55,355.40,388.60)` | `(0.5040,0.7372,0.0852,0.1464)` |
...
视觉模型使用 `yolov5/yolov5/runs/train/cube_exp/weights/best.pt` 导出的 ONNX，
类别顺序为 `food`、`daily`、`electronics`。模型及来源校验信息位于：

```text
src/car3/models/vision/
```
...
| 控制量 | 原上限 | 载物上限 |
| --- | ---: | ---: |
| 前进速度 | `0.60 m/s` | `0.48 m/s` |
| 转向速度 | `2.00 rad/s` | `1.60 rad/s` |
| 终点朝向速度 | `1.20 rad/s` | `0.96 rad/s` |
...
正式任务执行节点只使用摄像头完成物块区域选择和最终对位。数据采集器仍会读取
Gazebo 坐标，但用途仅限于离线生成训练/标定数据：
...
文件名追加样本。仓库当前样本集共 135 张，覆盖 3 个类别在左侧、中间、右侧的全部
9 种“类别 × 区域”组合，避免模型把物块类别错误地绑定到固定区域。输出结构可直接供
YOLOv5 使用：
...
然后重新执行 `source devel/setup.bash` 并启动任务。
...
在 RViz 添加 `Image` 显示并选择该话题。青色框是可夹取目标，绿色框是目标类别，
橙色框是其他类别；标签同时显示视觉复核结果、YOLO 原始类别和置信度。
